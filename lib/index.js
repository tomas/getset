...
Config.prototype.merge_data = function(what, obj, replace) {
  ...
  var key     = '_' + what,
      values  = helpers.unflatten(obj),
      current = helpers.unflatten(this[key]);

  var res  = helpers.mixin(current, values, replace);

  this[key] = helpers.flatten(res);
  this._modified = true;
  return this;
}
...
Config.prototype.get = function(key, subkey) {
  ...
  if (subkey) {
    ...
    else if (typeof this._values[key + '.' + subkey] != 'undefined')
      res = this._values[key + '.' + subkey];
    else
...
Config.prototype.sync = function(other_file, replace, cb) {
  ...
  types[this.type].read(other_file, function(err, result) {
    ...
    var removed = helpers.intersect(helpers.unflatten(self._values), helpers.unflatten(result.values));
    self._values = helpers.flatten(removed);
