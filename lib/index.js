...
var Config = function(opts) {
  ...
  this.readonly  = opts.readonly || false; // throws error when trying to set a new value
...
// set('foo')               # => throws error
// set('foo', 'bar')        # => { foo: 'bar' }
// set({ foo: 'bar' }, 123) # => throws error
// set('foo.bar', '123')    # => { foo: { bar: '123' } }

Config.prototype.set = function(key, val) {
  if (this.readonly)
    throw new Error('Readonly mode. Invalid call to .set()'); // undefined
  ...
  if (typeof key == 'string') {
    ...
    if (typeof val != 'undefined')
      throw new Error("Ambiguous call to .set. Don't know what to do with " + val);
  ...
  if (this.locked && !helpers.all_keys_exist(this._values, obj)) {
    debug('New keys must match existing keys in locked mode. Stopping here.')
    return; // undefined
  }
