...
function decode (str) {
  ...
  var parse_value = function(str) {
    var val = unsafe(str || "");
    if (val.trim().match(/^\[.*\]$/))
      return val.split(/,\s?/).map(function(e) { return e.replace(/\[|\]/, '').trim() });
    else
      return val;
  }
  ...
  lines.forEach(function (line, _, __) {
    ...
    var key = unsafe(match[2])
      , value = match[3] ? parse_value(match[4]) : true
  ...
  var data = {values: out, comments: comments };
